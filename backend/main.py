...
import db_conn
from SkynetEye import settings
...
def call(sys_args):
    if len(sys_args) == 0:
        ...
        if hasattr(feature_ins,sys_args[0]):
            ...
            print "\033;[31;1m Invalid argument! \033 [0m"
            feature_list()
...
class Features(object):
    ...
    def token_auth(self):
        ...
        while count < 3:
            ...
            token = raw_input("press Enter if you don't have toke,[input your token]:").strip()
    ...
    def __auth(self):
        ...
        while count < 3:
            ...
            try:
                ...
                sys.exit("\033[31;1m haven't set SkynetEye account yet ,please login http://localhost:8000/admin find 'SkynetEye账户' and create an account first!\033[0m")
        else:
    ...
    def fetch_hosts(self):
        ...
        while True:
            try:
                ...
                user_choice = raw_input("\033[32;1m SkynetEye>>: \033 [0m").strip()
                ...
                if user_choice.isdigit():
                    ...
                    if user_choice < len(host_groups):
                        while True:
                            hosts = models.BindHosts.objects.filter(host_group_id=host_groups[user_choice].id)
                            for index,h in enumerate(hosts):
                                print " %s.\t%s(%s) %s "%(index,h.host.hostname,h.host.ip_addr,h.host_user.username)
                            user_choice2 = raw_input("\003[32;1m ['b'(back)]>>>: \003[0m").strip()

                            if user_choice2.isdigit():
                                user_choice2 = int(user_choice2)
                                if user_choice2 < len(hosts):
                                    h = hosts[user_choice2]
                                    print '\033[32;1m---connnecting [%s] with user [%s]---\003[0m' % (h.host.ip_addr,h.host_user.username)
                                    try:
                                    ...
                                    print_msg("No this option!",'error')
                            elif user_choice2 == "b":
                                break
                    else:
                        print("No this option!","err")
